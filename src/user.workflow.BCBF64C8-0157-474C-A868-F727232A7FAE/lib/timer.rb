...
def set_alarm(target_time_string)
  now = Time.now
  target_seconds = seconds_from_string(target_time_string)
  
  target_time_today = Time.new(now.year, now.month, now.day) + target_seconds

  if target_time_today <= now
    target_time_today += 86_400 # Add 24 hours in seconds
  end

  run_seconds = (target_time_today - now).to_i
  
  set_timer("#{run_seconds}s")
  
  "Alarm set for #{Time.now.strftime('%H:%M')} is done!"
end
